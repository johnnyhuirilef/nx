  ...
  for (const [packageName, importerPath] of Object.entries(
    ...
    for (const depType of WORKSPACE_DEP_TYPES) {
      ...
      for (const [depName, ref] of Object.entries(deps)) {
        ...
        if (target) {
          // pnpm keys a package by its real name, so an aliased dependency's
          // ref carries the full key rather than the bare spec
          resolved[depName] =
            target === depName
      ...
      Object.keys(packageJson[depType]).forEach((packageName) => {
        ...
        if (workspaceTarget !== null) {
          ...
          if (workspaceTarget !== packageName) {
            // aliased entry: root importers may only reference the target
            // through aliases, so the by-name search below cannot find it; the
            // target's graph root is authoritative
          ...
          if (importerKeyForPackage) {
            ...
            // the directory package key's ref (no leading `./`). An aliased
            // ref carries the full key, since pnpm keys a package by its real
            // name.
