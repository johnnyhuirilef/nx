...
snapshots:
  ...
  lodash@4.17.21: {}`;
      ...
      // the alias key resolves to the target's directory package via a full
      // aliased ref (pnpm keys a package by its real name)
      expect(result).toMatch(
...
snapshots:
  ...
  lodash@4.17.21: {}`;
      ...
      // the aliased target still gets its own directory package
      expect(result).toContain(
        `'@myorg/lib-b@file:workspace_modules/@myorg/lib-b':`
      );
      // the alias key inside lib-a's snapshot resolves to the target's
      // directory package via a full aliased ref
      expect(result).toContain(
